        ...
        {steps
          ...
          .map((step, index) => (
            <li key={step.id} className="flex gap-3 sm:gap-4">
              <span className="bg-warm-100 font-heading text-warm-700 print:bg-ink-100 print:text-ink-900 flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full text-base font-semibold sm:h-8 sm:w-8 sm:text-lg">
                {index + 1}
              </span>
              <div className="flex-1 pt-1">
                ...
                {step.timer_minutes != null && step.timer_minutes > 0 ? (
                  <span className="bg-warm-100 text-warm-700 print:text-ink-600 mt-2 inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-sm font-medium print:bg-transparent print:px-0">
