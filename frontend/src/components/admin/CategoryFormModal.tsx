...
export function CategoryFormModal({
  ...
  // Key that forces form reset when modal opens or category changes
  const formKey = `${isOpen}-${category?.id ?? "new"}`;
...
function CategoryFormContent({
  ...
  const [iconName, setIconName] = useState(category?.icon_name ?? "plats");
      ...
      <Form onSubmit={handleSubmit} className="space-y-6">
        ...
        <TextField className="space-y-2">
          ...
          <Input
            ...
            className="border-ink-200 focus:border-warm-500 focus:ring-warm-500 w-full rounded-lg border px-3 py-2 text-base transition outline-none focus:ring-2 focus:ring-offset-0"
            autoFocus
          />
        </TextField>
